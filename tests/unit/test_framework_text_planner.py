...
def test_plan_framework_text_sets_button_label_and_parent_hint() -> None:
    result = plan_framework_text(
        framework="bmad",
        profile="cliente2",
        text="""
        Objective: Add login action.
        - Create a section named Login form inside root.
        - Add button labeled Continue inside Login form.
        """,
    )

    assert result["ok"] is True
    button = result["program"]["steps"][1]
    assert button["intent"] == "create_button"
    assert button["text"] == "Continue"
    assert button["parent"] == "Login form"
