...
def get_prompt(name: str, arguments: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    if name == "bubble-task-runbook":
        ...
        text = (
            ...
            "the task.\n\n"
            "If the user provided a profile and app id for setup, call `bubble_project_bootstrap` first. "
            "Otherwise call `bubble_profile_status` for the target profile. If the profile is unclear, call "
            "`bubble_profile_list` before asking the user. Then call `bubble_task_runbook` with the "
        ...
        text = (
            ...
            "`bubble_task_runbook` with an HTML import task. Resolve the target page/reusable with "
            "`bubble_context_find` using profile, exact=true, and include_metadata=false. Then call "
