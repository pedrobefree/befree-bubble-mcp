...
FIELD_LIBRARY: dict[str, JsonSchema] = {
    ...
    "plugin_key": _prop(
        "string",
        "Bubble plugin registry key to install. If the source dependency is an element/action type like progressbar-ProgressBar, pass that value and the MCP will use the prefix before the dash.",
        examples=["progressbar", "progressbar-ProgressBar", "1627152028063x152738721905770500-AAs"],
    ),
    "plugin_value": _prop(
    ...
    "installed_version": _prop(
        ...
        "Value to write to the plugin installed-version setting when include_installed_version=true. The Bubble editor used 1 for progressbar.",
        default=1,
        examples=[1, "2.0.0"],
    ),
    "installed_version_key": _prop(
        "string",
        "Optional explicit installed-version setting key. Defaults to <plugin_key>_installed_version.",
        examples=["progressbar_installed_version"],
    ),
    ...
    "post_check_conflicts": _prop(
        "boolean",
        "After a successful plugin write, call /appeditor/get_plugin_conflicts like the Bubble editor does.",
        default=True,
    ),
    "notify_ai_context_change": _prop(
        "boolean",
        "After a successful plugin write, call /appeditor/notify_ai_app_context_change like the Bubble editor does.",
        default=True,
    ),
    "id_counter": _prop(
        "integer",
        "Optional id_counter value to append to the Bubble write payload when reproducing a captured editor payload.",
        examples=[20000330],
    ),
    "app_json_path": _prop(
        "string",
    ...
    return [
        ...
        tool_schema(
            ...
            [
                ...
                "id_counter",
                "execute",
                "post_check_conflicts",
                "calculate_derived",
                "notify_ai_context_change",
            ],
            required=["profile", "plugin_key"],
        ),
        tool_schema(
            "bubble_execute_plan",
