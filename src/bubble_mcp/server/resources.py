...
def _agent_runtime_markdown() -> str:
    return "\n".join(
        [
            ...
            "",
            "1. `bubble_task_runbook` when the agent has a user task and needs route, recipe, safeguards, and compact tool matches in one call.",
...
def _agent_quickstart_markdown() -> str:
    return "\n".join(
        [
            ...
            "6. If you need exact arguments for one selected tool, read `bubble://tools/{tool_name}` instead",
            "   of loading the full `tools/list` response.",
            "7. When resolving or verifying a known page/element/context ref, prefer `bubble_context_find`",
            "   with `exact=true` and `include_metadata=false` before using broader searches.",
            "8. After real writes, verify with `bubble_context_detect`, changelog tools, or `bubble_runtime_smoke`.",
            "",
            "Setup assumptions:",
            "",
